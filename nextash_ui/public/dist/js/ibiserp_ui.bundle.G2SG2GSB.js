(() => {
  // frappe-html:/home/ali/bench-15/apps/nextash_ui/nextash_ui/public/js/ui/toolbar/navbar.html
  frappe.templates["navbar"] = `<header class="navbar navbar-expand sticky-top" role="navigation">
  <div class="container">
    <a class="navbar-brand navbar-home navbar-ibiserp" href="/app">
      <img
        class="app-logo"
        style="width: {{ navbar_settings.logo_width || 60 }}px"
        src="/assets/nextash_ui/images/logo.png"
      />
    </a>
            ...
            {% for item in navbar_settings.settings_dropdown %} 
              {% if (!item.hidden) { %} 
                {% if (item.item_label != "Toggle Theme") { %}
                  {% if (item.route) { %}
                  <a class="dropdown-item" href="{{ item.route }}">{%= __(item.item_label) %}</a>
                  {% } else if (item.action) { %}
                  <a class="dropdown-item" onclick="return {{ item.action }}">{%= __(item.item_label) %}</a>
                  {% } else { %}
...
`;
  ...
  // ../nextash_ui/nextash_ui/public/js/ui/toolbar/issue-report.js
  ...
  $(document).on("click", ".showHelpDialog", function() {
    new frappe.views.CommunicationComposer({
      ...
      recipients: "support@attitude-dynamics.com"
    });
  });
})();
//# sourceMappingURL=ibiserp_ui.bundle.G2SG2GSB.js.map
